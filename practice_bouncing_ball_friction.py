__author__ = 'torsten'
# A Bouncy Red Ball (with Friction)
...
BALL_RADIUS = 20
BALL_COLOR = 'red'
DRAG = 0.001 # friction due to air
DAMPEN = 0.1 # friction due to deformation
    ...
    def update(self):
        ...
        for d in range(2):
            ...
            if self.radius * 2 < self.pos[d] + self.vel[d] + self.acc[d] + gravity[d] + self.radius < FRAME_SIZE[d]:
                ...
                self.vel[d] -= self.vel[d] * DRAG
            else:
                self.vel[d] = -self.vel[d] * (1 - DAMPEN) # collision - invert velocity
