...
class Paddle:
    def __init__(self, pos, incr = 10):
        ...
        self.pos = [pos[0], pos[1] + self.length / 2]
        self.limit = game.screen_height() - self.length
    
    def update(self):
        if 0 < self.pos[1] + self.vel < self.limit :
            self.pos[1] += self.vel

    def draw(self, canvas):
        canvas.draw_line([self.pos[0], self.pos[1]], [self.pos[0], self.pos[1] + self.length], 
                         self.width, "White")
    def move_up(self):
        self.vel -= self.incr
